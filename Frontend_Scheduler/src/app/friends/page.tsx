...
export default function FriendsPage() {
  ...
  const tabs: { key: Tab; label: string; count: number }[] = [
    { key: 'friends', label: 'Friends', count: network.friends.length },
      ...
      <div className="max-w-[1024px] w-full mx-auto px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <div className="flex-1 w-full space-y-5">
            ...
            {/* List panel */}
            <div className={`${cardClass} flex flex-col h-[calc(100vh-220px)] lg:h-[calc(100vh-140px)]`}>
              ...
              {/* List content (Internally Scrollable) */}
                ...
                {networkLoading ? (
                  ...
                  <div className="py-16 text-center">
                    ...
                    <p className={`text-[12.5px] ${dark ? 'text-zinc-500' : 'text-zinc-400'}`}>
                      {!query && activeTab === 'friends' && 'Connect with other students to see them here'}
          ...
          {userDetails?.friendCode && (
            <div className="w-full lg:w-[340px] shrink-0 space-y-5">
              ...
              <div className={`p-5 rounded-xl border ${
                ...
                <div className={`border-t ${dark ? 'border-zinc-800/80' : 'border-zinc-150'} my-3`} />

                {/* Add Friend */}
                <div className="space-y-2">
                  <div>
                    <h3 className="text-[13px] font-semibold tracking-tight">Add by Friend Code</h3>
                    <p className={`text-[11px] ${muted} mt-0.5`}>Enter a friend's code to connect.</p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-3.5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-[12px] font-medium transition-all disabled:opacity-40 flex items-center justify-center gap-1.5 shrink-0"
                    >
                      {addingFriend ? <Loader2 size={12} className="animate-spin" /> : <UserPlus size={12} />}
                      <span>Add</span>
                    </button>
                  </div>
                </div>
