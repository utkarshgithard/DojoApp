...
export default function WeeklyScheduleSetup() {
  ...
  return (
    <div className={`relative py-20 px-10 min-h-screen transition duration-300  ${darkMode ? 'dark bg-gray-900 text-white' : 'bg-white text-black'}`}>
      <div className="relative z-10 max-w-5xl mx-auto p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {weekdays.map((day) => (
            <div
              key={day}
              className={`p-4 rounded-xl border-2 border-gray-400 dark:border-gray-600 text-md`}
            >
              <div className='flex justify-center'>
                <h3 className="capitalize font-semibold text-2xl mb-3">{day}</h3>
              </div>
              ...
              {schedule[day].map((item: any, idx: number) => (
                <div key={idx} className="grid justify-center grid-cols-1 md:grid-cols-3 gap-4 mb-4 items-center ">
                  <input
                    ...
                    className={`p-2 rounded border-2 border-gray-400 w-full ${darkMode ? 'bg-gray-800 text-white border-gray-600' : ''}`}
                  />
                  ...
                  <div className="flex gap-2">
                    <select
                      ...
                      className={`border p-2 rounded ${darkMode ? 'bg-gray-800 text-white border-gray-600' : ''}`}
                    >
