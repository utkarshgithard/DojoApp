...
export default function ChatLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const handleIncomingMessage = (message: any) => {
      ...
      if (message.id && String(message.userId) !== String(currentUserId)) {
        socket.emit("messageDelivered", { chatId: message.chatId, messageId: message.id });
      }
        ...
        <div className={`px-5 pt-[22px] pb-[14px] shrink-0`}>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-[36px] pr-3 py-[10px] text-[13.5px] rounded-xl outline-none transition-colors border ${dark ? "bg-zinc-950 border-zinc-800 text-white placeholder-zinc-600 focus:border-zinc-700" : "bg-[#F3F1FA] border-[#E7E3F3] text-[#15131F] placeholder-[#8D89A3] focus:border-[#D8D4EA]"}`}
            />
          </div>
        </div>
